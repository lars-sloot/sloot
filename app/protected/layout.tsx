...
export default async function ProtectedLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7f4] text-[#18221b] lg:grid lg:grid-cols-[250px_1fr]">
      ...
      <main>
        <header className="flex items-center justify-between border-b border-[#dde4de] bg-white px-6 py-4"><div><p className="text-sm text-[#718078]">Sloot 2Wielers</p><p className="font-medium">{isAdmin ? "Beheeromgeving" : "Pakbonnenomgeving"}</p></div><AuthButton/></header>
