...
import { createNoteItem, deleteNoteItem, updateNoteItem, type NoteItemState } from "@/app/actions/notes";
...
function ItemRow({ noteId, item }: { noteId: string; item: NoteItem }) {
  ...
  const [deleteState, deleteAction, deleting] = useActionState(deleteNoteItem, initialState);
  ...
  return <form action={action} className="grid gap-3 px-4 py-4 sm:grid-cols-[minmax(0,1fr)_7rem_auto] sm:items-start sm:px-5">
    ...
    <div className="flex gap-2 sm:mt-[1.375rem]">
      <button disabled={pending || deleting} className="inline-flex h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-[#cdd8d0] px-3 text-sm font-medium text-[#173b2b] disabled:opacity-60"><Save size={15}/>{pending ? "Opslaan…" : "Opslaan"}</button>
      <button type="submit" formAction={deleteAction} disabled={pending || deleting} onClick={(event) => { if (!window.confirm("Weet je zeker dat je deze artikelregel wilt verwijderen?")) event.preventDefault(); }} aria-label={`Artikelregel ${item.description || item.line_number} verwijderen`} title="Artikelregel verwijderen" className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-red-200 text-red-700 hover:bg-red-50 disabled:opacity-60"><Trash2 size={16}/></button>
    </div>
    ...
    {state.message || deleteState.message ? <p role="status" className={`text-xs sm:col-span-3 ${state.status === "error" || deleteState.status === "error" ? "text-red-700" : "text-emerald-700"}`}>{deleteState.message || state.message}</p> : null}
  </form>;
}
