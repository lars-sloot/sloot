...
type NotePage = { id: string; pageNumber: number; storagePath: string; mimeType: string; url: string | null };
...
export function NoteDetailPanel({ note, uploaderName, decisionActorName, branches, photoPages, closeHref, isAdmin }: { note: Note; uploaderName: string; decisionActorName: string | null; branches: Branch[]; photoPages: NotePage[]; closeHref: string; isAdmin: boolean }) {
  ...
  return <>
    ...
    <aside className="fixed inset-y-0 left-0 right-0 z-50 flex min-w-0 max-w-full flex-col overflow-hidden bg-[#f8faf8] shadow-2xl sm:left-auto sm:w-full sm:max-w-[620px] sm:border-l sm:border-[#dce4dd]">
      ...
      <div className="min-w-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-5 sm:px-7 sm:py-6">
        ...
        <div className="mb-6"><p className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-[#718078]">Originele {photoPages.length === 1 ? "pagina" : `${photoPages.length} pagina's`}</p><NotePhoto pages={photoPages} alt={`Pakbon ${note.delivery_number || note.id} van ${note.supplier || "onbekende leverancier"}`} deleted={Boolean(note.deleted_at)}/></div>

        <form action={formAction} className="grid gap-4 rounded-2xl border border-[#dce4dd] bg-white p-5 sm:grid-cols-2">
          <input type="hidden" name="id" value={note.id}/>
          <label className="grid min-w-0 gap-1.5 text-sm text-[#526057]"><span>Leverancier</span><input name="supplier" defaultValue={note.supplier || ""} placeholder="Leverancier" className="min-w-0 w-full rounded-xl border border-[#d6ddd7] px-3.5 py-2.5 text-[#17231d] outline-none focus:border-[#173b2b]"/></label>
          <label className="grid min-w-0 gap-1.5 text-sm text-[#526057]"><span>Pakbonnummer</span><input name="delivery_number" defaultValue={note.delivery_number || ""} placeholder="Pakbonnummer" className="min-w-0 w-full rounded-xl border border-[#d6ddd7] px-3.5 py-2.5 text-[#17231d] outline-none focus:border-[#173b2b]"/></label>
          <label className="grid min-w-0 gap-1.5 text-sm text-[#526057]"><span>Datum pakbon</span><input name="delivery_date" type="date" defaultValue={note.delivery_date || ""} className="min-w-0 w-full rounded-xl border border-[#d6ddd7] px-3.5 py-2.5 text-[#17231d] outline-none focus:border-[#173b2b]"/></label>
          <label className="grid min-w-0 gap-1.5 text-sm text-[#526057]"><span>Filiaal</span><select name="branch_id" defaultValue={note.branch_id} className="min-w-0 w-full rounded-xl border border-[#d6ddd7] bg-white px-3.5 py-2.5 text-[#17231d] outline-none focus:border-[#173b2b]">{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label>
          <label className="grid min-w-0 gap-1.5 text-sm text-[#526057] sm:col-span-2"><span>Omschrijving</span><textarea name="article_summary" defaultValue={note.article_summary || ""} rows={3} placeholder="Samenvatting artikelen" className="min-w-0 w-full resize-y rounded-xl border border-[#d6ddd7] px-3.5 py-2.5 text-[#17231d] outline-none focus:border-[#173b2b]"/></label>
